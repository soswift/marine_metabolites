...
make_chem_phyloseq <-
  function(chem_abund = chem_abund, chem_meta = chem_meta, id_column = "sample_barcode") {
...
do_permanova <- function(phyloseq_obj, var_name, dist_obj, description){
  ...
  aov_results <- adonis(a_formula, data = a_meta)
...
plot_NMDS <- function(a_phy, desc,  color_var, shape_var=NULL, dist_obj, dist_method = "bray"){
  ...
  ggsave(paste0("output/NMDS/",i,"_",desc,"_NMDS_",dist_method,"_by_", color_var, ".pdf"), plot = p, width = 7, height = 5)
  return(p)
}
                              ...
                              shape = "site_name")
{
  ...
  if(microbe_dist == "Unifrac"){
    ...
    ST.dist<- subset_dist(pair_micro_dist, microbe_phy)
  ...
  cols_to_keep <- c("proc.x", "proc.y", "safe_id", "site_name", "sample_type", "genus", "data_type")
